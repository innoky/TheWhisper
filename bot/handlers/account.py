...
<b>ID:</b> {user_info.get('id', 'N/A')}
<b>Имя:</b> {user_info.get('firstname', 'N/A')}
<b>Фамилия:</b> {user_info.get('lastname', 'N/A')}
<b>Username:</b> @{user_info.get('username', 'N/A')}
<b>Уровень:</b> ⭐ {user_info.get('level', 1)}/10
<b>Баланс:</b> 💰 {user_info.get('balance', 0)} т.
<b>Статус:</b> {'🚫 Заблокирован' if user_info.get('is_banned', False) else '✅ Активен'}

<b>🛒 Статистика покупок:</b>
• Куплено никнеймов: {len(user_pseudos)} шт.
"""
...
<b>Магазин никнеймов:</b>
• /market - магазин никнеймов для анонимных комментариев (оплата токенами)

<b>Комментарии:</b>
• Отправьте ссылку на пост с параметром для оставления анонимного комментария

<b>Предложка постов:</b>
• Просто отправьте пост в чат для рассмотрения администрацией

<b>💡 Подсказка:</b>
Используйте /account для просмотра вашего баланса токенов и купленных никнеймов!
"""
